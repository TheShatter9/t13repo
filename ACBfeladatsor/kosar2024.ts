...
//2. feladat: Olvassa be az eredmenyek. js állomány sorait és tárolja az adatokat egy olyan összetett adatszerkezetben,amely használatával a további feladatok megoldhatók!
...
function ObjektumFeltolto(feltoltendoElem: string[]): ACB[] {
    let beolvasottAdatok: ACB[] = [];
    for (let i = 0; i < feltoltendoElem.length; i++) {
        ...
        let aktEredmeny: ACB = {
            ...
            hazaiPont: Number(daraboltAdatok[2]),
            idegenPont: Number(daraboltAdatok[3]),
...
var acbAdatok: ACB[] = ObjektumFeltolto(eredmenyek);
console.log(acbAdatok);


//3. feladat: Határozza meg és írja ki a képernyőre a minta szerint, hogy a Real Madrid hány mérkőzést játszott hazai, illetve idegen csapatként!

function RealMadrid(vizsgaltTomb: ACB[]): [number, number] {
    let meccsek: [number, number] = [0, 0];
...
function RealMadridKiir(meccsekSzama: [number, number]): void {
    console.log("3. feladat: Real Madrid: Hazai: " + meccsekSzama[0] + " Idegen: " + meccsekSzama[1]);

}
RealMadridKiir(RealMadrid(acbAdatok));


//4. feladat: Határozza meg és írja ki a minta szerint, hogy volt-e döntetlen mérkőzés!

function dontetlen(vizsgaltTomb: ACB[]): string {
    let visszajelzoUzenet: string = "";
    let dontetlen: number = 0;
    let hazai: number = 0;
    let idegen: number = 0;

    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        if (vizsgaltTomb[i].hazaiPont == vizsgaltTomb[i].idegenPont) {
            dontetlen++;
        }
        else if (vizsgaltTomb[i].hazaiPont > vizsgaltTomb[i].idegenPont) {
            hazai++;
        }
        else {
            idegen++;
    ...
    if (hazai > idegen) {
        visszajelzoUzenet = "Volt döntetlen? nem";
    }
    else if (hazai < idegen) {
        visszajelzoUzenet = "Volt döntetlen? nem";
    }
    else {
        visszajelzoUzenet = "Volt döntetlen? igen";
    }
    return visszajelzoUzenet;
}

function dontetlenKiir(eredmeny: string): void {
    console.log("4. feladat: " + eredmeny);
}

dontetlenKiir(dontetlen(acbAdatok));


//5. feladat: Határozza meg és írja ki a minta szerint, hogy a barcelonai csapatnak mi a pontos neve!

function barcelona(vizsgaltTomb: ACB[]): string {
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        if (vizsgaltTomb[i].hazai.includes("Barcelona")) {
            return vizsgaltTomb[i].hazai;
        }
    }
    return "Nem található olyan csapat, amely Barcelona néven játszik.";
}

function barcelonaKiir(eredmeny: string): void {
    console.log("5. feladat: barcelonai csapat neve: " + eredmeny);
}

barcelonaKiir(barcelona(acbAdatok));


//6. feladat: Határozza meg és írja ki a minta szerint, hogy 2004. november 21-én mely csapatok játszottak mérkőzéseket, és milyen eredmény született!

function novemberiMeccsek(vizsgaltTomb: ACB[]): string[] {
    let meccsek: string[] = [];
    let datum: string = "2004-11-21";
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        if (vizsgaltTomb[i].idopont == datum) {
            meccsek.push(vizsgaltTomb[i].hazai + "-" + vizsgaltTomb[i].idegen + " (" + vizsgaltTomb[i].hazaiPont + ":" + vizsgaltTomb[i].idegenPont + ")");
        }
    }
    return meccsek;
...
function novemberiMeccsekKiir(kiirandoErtek: string[]): void {
    console.log("6. feladat:")
    console.log(kiirandoErtek);
}
novemberiMeccsekKiir(novemberiMeccsek(acbAdatok));

//7. feladat: Határozza meg és írja ki a minta szerint, hogy melyek azok a stadionok, amelyek 20-nál több alkalommal voltak kosárlabdamérkőzések helyszínei! 
//A stadionok neve mögött jelenjen meg a mérkőzések száma is! A feltételnek megfelelő stadionok tetszőleges sorrendben jelenhetnek meg.

function legtobbStadion(vizsgaltTomb: ACB[]): string[] {
    let stadionok: string[] = [];
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        if (vizsgaltTomb[i].helyszin)
    }
}